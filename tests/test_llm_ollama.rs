...
async fn test_ollama_generates_spanish_interview_response() -> Result<()> {
    ...
    println!("Testing LLM connection to: {} with model: {}", config.openai_base_url, config.openai_model);
    ...
    assert!(has_spanish_chars, "Response should contain Spanish characters");

    // Should mention Rust, developer, or interview context
    let keywords = ["Rust", "desarrollador", "ingeniero", "experiencia", "técnica", "entrevista"];
    let has_keyword = keywords.iter().any(|k| response.contains(k));
    assert!(has_keyword, "Response should contain at least one interview-relevant keyword. Got: {:.80}", response);
